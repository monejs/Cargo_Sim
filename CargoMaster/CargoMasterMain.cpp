...
CargoMasterFrame::CargoMasterFrame(wxWindow* parent,wxWindowID id)
{
    ...
    ConstantsGrid->SetColLabelValue(5, _("X1 f. Ap., m"));
    ConstantsGrid->SetColLabelValue(6, _("X2 f. Ap., m"));
...
void CargoMasterFrame::OnLoadButtonSelected(wxCommandEvent& event)
{
    ...
    if (dialog.ShowModal()==wxID_OK){
        if (!ShipBody.load(dialog.GetPath().ToStdString()))
        {
            ...
            for(int i=0; i!=50; i++)
            {
                ConstantsGrid->SetCellValue(i,0,ShipBody.read_con_name(i));
                ConstantsGrid->SetCellValue(i,1,wxString::Format(wxT("%.2f"),ShipBody.read_con_weight(i)));
                ConstantsGrid->SetCellValue(i,2,wxString::Format(wxT("%.2f"),ShipBody.read_con_lcg(i)));
                ConstantsGrid->SetCellValue(i,3,wxString::Format(wxT("%.2f"),ShipBody.read_con_vcg(i)));
                ConstantsGrid->SetCellValue(i,4,wxString::Format(wxT("%.2f"),ShipBody.read_con_tcg(i)));
                ConstantsGrid->SetCellValue(i,5,wxString::Format(wxT("%.2f"),ShipBody.read_con_start(i)));
                ConstantsGrid->SetCellValue(i,6,wxString::Format(wxT("%.2f"),ShipBody.read_con_end(i)));
            }
            for (int i=0; i!=50; i++)
            {
                DeadLoadGrid->SetCellValue(i,0,ShipBody.read_car_name(i));
                DeadLoadGrid->SetCellValue(i,1,wxString::Format(wxT("%.2f"), ShipBody.read_car_weight(i)));
                DeadLoadGrid->SetCellValue(i,2,wxString::Format(wxT("%.2f"), ShipBody.read_car_lcg(i)));
                DeadLoadGrid->SetCellValue(i,3,wxString::Format(wxT("%.2f"), ShipBody.read_car_vcg(i)));
                DeadLoadGrid->SetCellValue(i,4,wxString::Format(wxT("%.2f"), ShipBody.read_car_tcg(i)));
                DeadLoadGrid->SetCellValue(i,5,wxString::Format(wxT("%.2f"), ShipBody.read_car_length(i)));
                DeadLoadGrid->SetCellValue(i,6,wxString::Format(wxT("%.2f"), ShipBody.read_car_breadth(i)));
                DeadLoadGrid->SetCellValue(i,7,wxString::Format(wxT("%.2f"), ShipBody.read_car_height(i)));
            }
...
void CargoMasterFrame::OnConstantsGridCellChanged(wxGridEvent& event)
{
    if (ConstantsGrid->GetGridCursorCol()==0)
    {
        ...
        {
            ConstantsGrid->SetCellTextColour(ConstantsGrid->GetGridCursorRow(),0,wxNullColour);
        }else{
    ...
    if (ConstantsGrid->GetGridCursorCol()==1)
    {
        ...
        ConstantsGrid->SetCellTextColour(ConstantsGrid->GetGridCursorRow(),1,wxNullColour);
    }else{
    ...
    {
        ConstantsGrid->SetCellTextColour(ConstantsGrid->GetGridCursorRow(),2,wxNullColour);
    }else{
    ...
    if (ConstantsGrid->GetGridCursorCol()==3)
    {
        ...
        ConstantsGrid->SetCellTextColour(ConstantsGrid->GetGridCursorRow(),3,wxNullColour);
    }else{
    ...
    if (ConstantsGrid->GetGridCursorCol()==4)
    {
        ...
        ConstantsGrid->SetCellTextColour(ConstantsGrid->GetGridCursorRow(),4,wxNullColour);
    }else{
        ConstantsGrid->SetCellTextColour(ConstantsGrid->GetGridCursorRow(),4,*wxRED);
        return;
        }
    }
    if (ConstantsGrid->GetGridCursorCol()==5)
    {
        if (ShipBody.set_con_start(ConstantsGrid->GetGridCursorRow(),CellVal(ConstantsGrid).ToStdString()))
        {
    ...
    if (ConstantsGrid->GetGridCursorCol()==6)
    {
        if (ShipBody.set_con_end(ConstantsGrid->GetGridCursorRow(),CellVal(ConstantsGrid).ToStdString()))
        {
...
void CargoMasterFrame::OnDeadLoadGridCellChanged(wxGridEvent& event)
{
    if (DeadLoadGrid->GetGridCursorCol()==0)
    {
        ...
        {
            DeadLoadGrid->SetCellTextColour(DeadLoadGrid->GetGridCursorRow(), DeadLoadGrid->GetGridCursorCol(), wxNullColour);
        }else{
...
{
     ShipBody.save(ShipBody.read_s_name()+" Condition at "+ShipBody.currentDateTime());
}
